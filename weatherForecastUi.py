# -*- coding: utf-8 -*-

################################################################################
## Form generated from reading UI file 'weather_forecast.ui'
##
## Created by: Qt User Interface Compiler version 6.7.2
##
## WARNING! All changes made in this file will be lost when recompiling UI file!
################################################################################

from PySide6.QtCore import (QCoreApplication, QDate, QDateTime, QLocale,
    QMetaObject, QObject, QPoint, QRect,
    QSize, QTime, QUrl, Qt)
from PySide6.QtGui import (QBrush, QColor, QConicalGradient, QCursor,
    QFont, QFontDatabase, QGradient, QIcon,
    QImage, QKeySequence, QLinearGradient, QPainter,
    QPalette, QPixmap, QRadialGradient, QTransform)
from PySide6.QtWidgets import (QApplication, QGridLayout, QHBoxLayout, QLabel,
    QLineEdit, QMainWindow, QMenuBar, QPushButton,
    QSizePolicy, QStatusBar, QTabWidget, QVBoxLayout,
    QWidget)

class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        if not MainWindow.objectName():
            MainWindow.setObjectName(u"MainWindow")
        MainWindow.resize(743, 810)
        MainWindow.setMaximumSize(QSize(16777215, 16777215))
        self.centralwidget = QWidget(MainWindow)
        self.centralwidget.setObjectName(u"centralwidget")
        self.horizontalLayout_3 = QHBoxLayout(self.centralwidget)
        self.horizontalLayout_3.setObjectName(u"horizontalLayout_3")
        self.tabWidget = QTabWidget(self.centralwidget)
        self.tabWidget.setObjectName(u"tabWidget")
        self.tab = QWidget()
        self.tab.setObjectName(u"tab")
        self.horizontalLayout_2 = QHBoxLayout(self.tab)
        self.horizontalLayout_2.setObjectName(u"horizontalLayout_2")
        self.widget = QWidget(self.tab)
        self.widget.setObjectName(u"widget")
        self.widget.setMaximumSize(QSize(500, 700))
        self.verticalLayout_3 = QVBoxLayout(self.widget)
        self.verticalLayout_3.setObjectName(u"verticalLayout_3")
        self.horizontalLayout = QHBoxLayout()
        self.horizontalLayout.setObjectName(u"horizontalLayout")
        self.horizontalLayout.setContentsMargins(10, 10, 10, 10)
        self.label = QLabel(self.widget)
        self.label.setObjectName(u"label")

        self.horizontalLayout.addWidget(self.label)

        self.locationInput = QLineEdit(self.widget)
        self.locationInput.setObjectName(u"locationInput")
        self.locationInput.setMaximumSize(QSize(293, 16777215))

        self.horizontalLayout.addWidget(self.locationInput)

        self.locationSubmit = QPushButton(self.widget)
        self.locationSubmit.setObjectName(u"locationSubmit")

        self.horizontalLayout.addWidget(self.locationSubmit)


        self.verticalLayout_3.addLayout(self.horizontalLayout)

        self.verticalLayout = QVBoxLayout()
        self.verticalLayout.setObjectName(u"verticalLayout")
        self.location = QLabel(self.widget)
        self.location.setObjectName(u"location")
        self.location.setAlignment(Qt.AlignmentFlag.AlignCenter)

        self.verticalLayout.addWidget(self.location)

        self.gridLayout = QGridLayout()
        self.gridLayout.setObjectName(u"gridLayout")
        self.gridLayout.setVerticalSpacing(10)
        self.latitude = QLabel(self.widget)
        self.latitude.setObjectName(u"latitude")

        self.gridLayout.addWidget(self.latitude, 0, 1, 1, 1)

        self.sunset = QLabel(self.widget)
        self.sunset.setObjectName(u"sunset")

        self.gridLayout.addWidget(self.sunset, 3, 1, 1, 1)

        self.label_8 = QLabel(self.widget)
        self.label_8.setObjectName(u"label_8")

        self.gridLayout.addWidget(self.label_8, 0, 0, 1, 1)

        self.sunrise = QLabel(self.widget)
        self.sunrise.setObjectName(u"sunrise")

        self.gridLayout.addWidget(self.sunrise, 2, 1, 1, 1)

        self.label_16 = QLabel(self.widget)
        self.label_16.setObjectName(u"label_16")

        self.gridLayout.addWidget(self.label_16, 3, 0, 1, 1)

        self.label_10 = QLabel(self.widget)
        self.label_10.setObjectName(u"label_10")

        self.gridLayout.addWidget(self.label_10, 1, 0, 1, 1)

        self.longitude = QLabel(self.widget)
        self.longitude.setObjectName(u"longitude")

        self.gridLayout.addWidget(self.longitude, 1, 1, 1, 1)

        self.label_14 = QLabel(self.widget)
        self.label_14.setObjectName(u"label_14")

        self.gridLayout.addWidget(self.label_14, 2, 0, 1, 1)

        self.label_2 = QLabel(self.widget)
        self.label_2.setObjectName(u"label_2")

        self.gridLayout.addWidget(self.label_2, 4, 0, 1, 1)

        self.currenttime = QLabel(self.widget)
        self.currenttime.setObjectName(u"currenttime")

        self.gridLayout.addWidget(self.currenttime, 4, 1, 1, 1)


        self.verticalLayout.addLayout(self.gridLayout)

        self.verticalLayout.setStretch(1, 1)

        self.verticalLayout_3.addLayout(self.verticalLayout)

        self.verticalLayout_2 = QVBoxLayout()
        self.verticalLayout_2.setObjectName(u"verticalLayout_2")
        self.description = QLabel(self.widget)
        self.description.setObjectName(u"description")
        self.description.setAlignment(Qt.AlignmentFlag.AlignCenter)

        self.verticalLayout_2.addWidget(self.description)

        self.dayornight = QLabel(self.widget)
        self.dayornight.setObjectName(u"dayornight")
        self.dayornight.setAlignment(Qt.AlignmentFlag.AlignCenter)

        self.verticalLayout_2.addWidget(self.dayornight)

        self.gridLayout_2 = QGridLayout()
        self.gridLayout_2.setObjectName(u"gridLayout_2")
        self.label_3 = QLabel(self.widget)
        self.label_3.setObjectName(u"label_3")

        self.gridLayout_2.addWidget(self.label_3, 0, 0, 1, 1)

        self.temp = QLabel(self.widget)
        self.temp.setObjectName(u"temp")

        self.gridLayout_2.addWidget(self.temp, 0, 1, 1, 1)

        self.label_7 = QLabel(self.widget)
        self.label_7.setObjectName(u"label_7")

        self.gridLayout_2.addWidget(self.label_7, 1, 0, 1, 1)

        self.maxtemp = QLabel(self.widget)
        self.maxtemp.setObjectName(u"maxtemp")

        self.gridLayout_2.addWidget(self.maxtemp, 1, 1, 1, 1)

        self.label_9 = QLabel(self.widget)
        self.label_9.setObjectName(u"label_9")

        self.gridLayout_2.addWidget(self.label_9, 2, 0, 1, 1)

        self.mintemp = QLabel(self.widget)
        self.mintemp.setObjectName(u"mintemp")

        self.gridLayout_2.addWidget(self.mintemp, 2, 1, 1, 1)

        self.label_17 = QLabel(self.widget)
        self.label_17.setObjectName(u"label_17")

        self.gridLayout_2.addWidget(self.label_17, 3, 0, 1, 1)

        self.uvmax = QLabel(self.widget)
        self.uvmax.setObjectName(u"uvmax")

        self.gridLayout_2.addWidget(self.uvmax, 3, 1, 1, 1)

        self.label_4 = QLabel(self.widget)
        self.label_4.setObjectName(u"label_4")

        self.gridLayout_2.addWidget(self.label_4, 4, 0, 1, 1)

        self.humidity = QLabel(self.widget)
        self.humidity.setObjectName(u"humidity")

        self.gridLayout_2.addWidget(self.humidity, 4, 1, 1, 1)

        self.label_5 = QLabel(self.widget)
        self.label_5.setObjectName(u"label_5")

        self.gridLayout_2.addWidget(self.label_5, 5, 0, 1, 1)

        self.feelslike = QLabel(self.widget)
        self.feelslike.setObjectName(u"feelslike")

        self.gridLayout_2.addWidget(self.feelslike, 5, 1, 1, 1)

        self.label_6 = QLabel(self.widget)
        self.label_6.setObjectName(u"label_6")

        self.gridLayout_2.addWidget(self.label_6, 6, 0, 1, 1)

        self.precip = QLabel(self.widget)
        self.precip.setObjectName(u"precip")

        self.gridLayout_2.addWidget(self.precip, 6, 1, 1, 1)

        self.label_11 = QLabel(self.widget)
        self.label_11.setObjectName(u"label_11")

        self.gridLayout_2.addWidget(self.label_11, 7, 0, 1, 1)

        self.pressure = QLabel(self.widget)
        self.pressure.setObjectName(u"pressure")

        self.gridLayout_2.addWidget(self.pressure, 7, 1, 1, 1)

        self.label_15 = QLabel(self.widget)
        self.label_15.setObjectName(u"label_15")

        self.gridLayout_2.addWidget(self.label_15, 8, 0, 1, 1)

        self.windspeed = QLabel(self.widget)
        self.windspeed.setObjectName(u"windspeed")

        self.gridLayout_2.addWidget(self.windspeed, 8, 1, 1, 1)


        self.verticalLayout_2.addLayout(self.gridLayout_2)

        self.verticalLayout_2.setStretch(2, 1)

        self.verticalLayout_3.addLayout(self.verticalLayout_2)

        self.verticalLayout_3.setStretch(1, 1)
        self.verticalLayout_3.setStretch(2, 2)

        self.horizontalLayout_2.addWidget(self.widget)

        self.tabWidget.addTab(self.tab, "")
        self.forecast = QWidget()
        self.forecast.setObjectName(u"forecast")
        self.horizontalLayout_6 = QHBoxLayout(self.forecast)
        self.horizontalLayout_6.setObjectName(u"horizontalLayout_6")
        self.widget_3 = QWidget(self.forecast)
        self.widget_3.setObjectName(u"widget_3")
        self.graphContainer = QVBoxLayout(self.widget_3)
        self.graphContainer.setObjectName(u"graphContainer")

        self.horizontalLayout_6.addWidget(self.widget_3)

        self.tabWidget.addTab(self.forecast, "")

        self.horizontalLayout_3.addWidget(self.tabWidget)

        MainWindow.setCentralWidget(self.centralwidget)
        self.menubar = QMenuBar(MainWindow)
        self.menubar.setObjectName(u"menubar")
        self.menubar.setGeometry(QRect(0, 0, 743, 22))
        MainWindow.setMenuBar(self.menubar)
        self.statusbar = QStatusBar(MainWindow)
        self.statusbar.setObjectName(u"statusbar")
        MainWindow.setStatusBar(self.statusbar)
#if QT_CONFIG(shortcut)
        self.label.setBuddy(self.locationInput)
#endif // QT_CONFIG(shortcut)

        self.retranslateUi(MainWindow)
        self.locationInput.returnPressed.connect(self.locationSubmit.click)

        self.tabWidget.setCurrentIndex(0)


        QMetaObject.connectSlotsByName(MainWindow)
    # setupUi

    def retranslateUi(self, MainWindow):
        MainWindow.setWindowTitle(QCoreApplication.translate("MainWindow", u"MainWindow", None))
        self.label.setText(QCoreApplication.translate("MainWindow", u"Enter a city:", None))
        self.locationSubmit.setText(QCoreApplication.translate("MainWindow", u"Search", None))
        self.location.setText(QCoreApplication.translate("MainWindow", u"Location", None))
        self.latitude.setText("")
        self.sunset.setText("")
        self.label_8.setText(QCoreApplication.translate("MainWindow", u"Latitude:", None))
        self.sunrise.setText("")
        self.label_16.setText(QCoreApplication.translate("MainWindow", u"Sunset:", None))
        self.label_10.setText(QCoreApplication.translate("MainWindow", u"Longitude:", None))
        self.longitude.setText("")
        self.label_14.setText(QCoreApplication.translate("MainWindow", u"Sunrise:", None))
        self.label_2.setText(QCoreApplication.translate("MainWindow", u"Time of Measurement (local tz): ", None))
        self.currenttime.setText("")
        self.description.setText("")
        self.dayornight.setText("")
        self.label_3.setText(QCoreApplication.translate("MainWindow", u"Current Temperature:", None))
        self.temp.setText("")
        self.label_7.setText(QCoreApplication.translate("MainWindow", u"Maximum Temperature:", None))
        self.maxtemp.setText("")
        self.label_9.setText(QCoreApplication.translate("MainWindow", u"Minimum Temperature:", None))
        self.mintemp.setText("")
        self.label_17.setText(QCoreApplication.translate("MainWindow", u"Maximum UV Index:", None))
        self.uvmax.setText("")
        self.label_4.setText(QCoreApplication.translate("MainWindow", u"Humidity:", None))
        self.humidity.setText("")
        self.label_5.setText(QCoreApplication.translate("MainWindow", u"Feels like:", None))
        self.feelslike.setText("")
        self.label_6.setText(QCoreApplication.translate("MainWindow", u"Precipitation:", None))
        self.precip.setText("")
        self.label_11.setText(QCoreApplication.translate("MainWindow", u"Pressure:", None))
        self.pressure.setText("")
        self.label_15.setText(QCoreApplication.translate("MainWindow", u"Wind Speed:", None))
        self.windspeed.setText("")
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab), QCoreApplication.translate("MainWindow", u"Current Weather", None))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.forecast), QCoreApplication.translate("MainWindow", u"Weather Forecast", None))
    # retranslateUi

